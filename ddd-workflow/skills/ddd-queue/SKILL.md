...
6. 閱讀 Queue Intake Review、Agent Communication Ledger、指定 item 的 communication_entries、依賴 item 的 handoff_summary、implemented_doc、commit 摘要與測試記錄，建立本 item 所需上下文。
...
16. commit 後確認工作樹乾淨。
17. 最終回覆只回報 item 狀態、commit hash、執行測試命令、ledger entries 與任何限制。不要開始下一個 item。
```
