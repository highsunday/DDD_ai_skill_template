...
一個新的輕量技能（`ddd-start`）：
1. 接收人類的原始請求
2. 偵測其類型：新功能 / 重構 / 錯誤 / 模糊想法
3. 路由到正確的入口點（grill-me → doc-maintain，或 triage → BXX 等）
4. 攜帶正確上下文進行交接
...
4. **技能是可組合的，而非整體性的。** 每個 Pocock 技能都可以獨立使用或作為 DDD 循環的一部分——整合是增量的，而非重寫。
5. **新工程師僅憑文檔即可完成入職。** `CONTEXT.md` + 模組文檔 + FXX 歷史記錄完整呈現全貌，無需翻閱程式碼庫。
